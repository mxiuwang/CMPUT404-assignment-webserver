...
** Requirements
   ...
   2 [ ] The webserver can be run using the runner.sh file
   3 [ ] The webserver can pass all the tests in freetests.py
   ...
   5 [ ] The webserver supports mime-types for HTML
   6 [ ] The webserver supports mime-types for CSS
   7 [ ] The webserver can return index.html from directories (paths
     that end in /)
   8 [ ] The webserver can server 404 errors for paths not found
   9 [ ] The webserver works with Firefox and Chromium
     http://127.0.0.1:8080/
   10 [ ] The webserver can serve CSS properly so that the front page
